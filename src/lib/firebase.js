...
export const getAllPlayers = async () => {
  const snap = await getDocs(query(collection(db, 'profiles'), where('role', '!=', 'admin')))
...
export const createCaptainCode = (code, label) =>
  setDoc(doc(db, 'captain_codes', code), {
    code, label, created_at: serverTimestamp()
  })
...
export const getTeamWeeklyStats = async (teamId, week, year) => {
  const snap = await getDoc(doc(db, 'team_weekly_stats', teamStatsId(teamId, week, year)))
  return snap.exists() ? { id: snap.id, ...snap.data() } : null
}
...
export const upsertWeeklySquad = (teamId, week, year, playerIds, positions = {}) =>
  setDoc(doc(db, 'weekly_squads', squadId(teamId, week, year)), {
    team_id: teamId, week_number: week, year,
...
export const upsertStats = (playerId, week, year, stats) =>
  setDoc(doc(db, 'weekly_stats', statsId(playerId, week, year)), {
    player_id: playerId, week_number: week, year,
    goals: stats.goals || 0,
...
export const upsertAward = (awardType, playerId, week, year) =>
  setDoc(doc(db, 'awards', `${awardType}_w${week}_${year}`), {
    award_type: awardType, player_id: playerId,
    week_number: week, year, updated_at: serverTimestamp(),
  })

// ---------- CAPTAIN RATINGS ----------
const ratingId = (fromId, toId, week, year) => `${fromId}_to_${toId}_w${week}_${year}`

export const upsertRating = (fromId, toId, week, year, pass, shoot, def, drib) =>
  setDoc(doc(db, 'captain_ratings', ratingId(fromId, toId, week, year)), {
    from_captain_id: fromId, to_player_id: toId,
    week_number: week, year,
    passing: pass, shooting: shoot, defending: def, dribbling: drib,
    updated_at: serverTimestamp(),
  })
...
export const getAllRatings = async (week, year) => {
  const snap = await getDocs(query(
    collection(db, 'captain_ratings'),
    where('week_number', '==', week),
    where('year', '==', year)
...
export const deleteInviteCodeById = async (
  code
) => {
